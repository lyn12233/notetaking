...
def: $s_m(t) = A\cos (\omega_c t+\varphi(t))$, 对相位调制 $\varphi(t) = K_P m(t)$; 对角度调制 $\varphi(t) = K_F m(t)$; 定义调频指数 $m_f = \Delta \varphi = K_F A_m / \omega_m = \Delta \omega / \omega_m\quad (\mathrm{单音调制下})$, 频偏比 $D = \Delta f / f$; 按调频指数分类, 窄带调频NBFM $m_f \ll \frac {\pi} 6$; 反之为宽带调频WBFM;
