...
theorem1: (**第一同构定理**) 设 $\overline{\varphi}: G/\ker \varphi \to \mathrm{Im} \varphi, \overline{\varphi}(\overline{x})=\varphi(x)$ 是群同态，则有同构; 特别 $|G|=|\ker\varphi||\mathrm{Im}\varphi|$

theorem2: (**第二同构定理**) 设 $H<G,\, N$, 则 $HN$ 是群, $N \triangleleft HN$, 且 $(HN)/N \cong H/(H\cap N)$
