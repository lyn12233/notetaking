...
theorem1-4：
  - (余数定理) $X-c$ 除 $f(X)$ 余数为 $r=f(c)$
  - (因子定理) $(X-c)|f(X)$ 当且仅当 $f(c)=0$
  - $n$ 次非零多项式最多有 $n$ 个根
  - $f,g \in F[x]$ 次数小于 $n$ , $f,g$ 个不同点取值相同, 则 $f=g$
  -  (lagrange插值公式) $\sum b_i \prod_{j\ne i} (X-a_j)/(a_i-a_j)$

def2：微商 $f'(X)=na_n X^{n-1}+...+a_1$

theorem5:
  - $c$ 为 $f(X)$ 重根 $\leftrightarrow$ $f(c)=f'(c)=0$ $\leftrightarrow$ $c$ 为 $(f(X),f'(X))$ 的根
  - $(f(X).f'(X))=1$, 则 $f(X)$ 无重根
  - $p(X)$ 不可约， 若 $p'(x)=0$ 则无重根，否则根都是重根
  - 、
theorem6：$F$ 为无限域时, 多项式形式和多项式函数存在环同构 $F[x]\cong f[x],\; f(X)\to f(x)$， 否则不同构

theorem7: 部分分式分解: 有理式 $\frac g f \in F(X)$ 可写为 $\frac g f = \frac{h_1}{p_1^{s_1}}+...+\frac{h_n}{p_n^{s_n}}$
 其中 $\deg h_i < \deg p_i^{s_i}$, $p_i$ 为互异的首一不可约多项式; 可以进一步分解为 $\frac{h}{p_s}=\frac{u_1}{p^1}+...$

lemma1: 设 $g(X)\in F[X]$ 不是常数， 则 $f$ 可以写为 $f=a_0+...+a_mg^m$

### 5.2 整系数多项式环

def: 本原多项式: 系数互素的多项式

theorem1:  次复系数多项式可以唯一分解为一次因子之积
theorem2:  次实系数多项式可以唯一分解为一次和二次不可约因子之积

lemma1: (容量分解) $\forall f(X)\in Q[X],\, f(X)=c_f^*(X),\, c_f \in Q,\, f^* \in Z[X]$

lemma2: 本原多项式之积为本原多项式

theorem3: $f=c_f g^* h^*$ 

ext1: $f(X)=a_nX^n+...+a_0 \in Z[X]$ 的有理根 $b/a$ 满足 $a|a_n,\; b|a_0$

theorem4: Eiseinstein判别法: 若素数 $p\nmid a_n,\; p|a_i(i=0,..,n-1),\;p^2\nmid a_0$ 则 $f(X)\in Z[X]$ 不可约

## 5.3 对称多项式

def1: 互异不定元的多项式形式: $F[X_1,...,X_n]=F[X_1,...][X_n]$; 字典排序法; 多项式各项排序的首项记为 $L(f)$

lemma1: $L(fg)=L(f)L(g)$

def2: 对多项式, 对换 $X_i,X_j$ 不变, 则称为对称多项式

考虑 $f(X)=\prod (X-X_i) =\sum (-1)^ksigma_kX^{n-k}$ X的系数满足韦达公式, 称为 $X_1,...,X_n$ 的初等对称多项式

theorem1: (对称多项式基本定理) $f(X_1,...,X_n)=\varphi(\sigma_1,...,\sigma_n)$

考虑 $P(X)=X^n-a_1X^{n-1}...+(-1)^na_n$, 根为 $x_1,...,x_n$, 记**判别式** $D_P=\prod_{i<j}(x_i-x_j)^2$, 则 $D_P(x_1),...,D_P(x_2)$ 为对称多项式, $D_P=0\leftrightarrow$ $P(X)$ 有重根
